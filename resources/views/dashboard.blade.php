                ...
                <div class="bg-white rounded-lg shadow-lg p-6 mt-6">
                    ...
                    <div class="space-y-3">
                        @forelse ($myContract ?? [] as $contract)
                            <div class="flex items-center justify-between p-3 rounded {{ $contract->is_active ? 'bg-green-100' : 'bg-gray-50' }}">
                                <div class="flex-1">
                                    ...
                                    <p class="text-sm text-gray-600">
                                        End Date: {{ $contract->tgl_selesai_kontrak ? $contract->tgl_selesai_kontrak->format('d M Y') : 'Ongoing' }}
                                    </p>
                                </div>
                                <div class="text-right">
                                    <p class="text-sm text-gray-600">
                                        Duration: {{ $contract->tgl_mulai_kontrak->locale('id')->diffForHumans($contract->tgl_selesai_kontrak ?? now(), ['parts' => 3, 'short' => false, 'syntax' => \Carbon\CarbonInterface::DIFF_ABSOLUTE]) }}
                                    </p>
                                    
                                    <p class="text-sm text-gray-600">Remaining Duration: 
                                        @if($contract->tgl_selesai_kontrak)
                                            ...
                                            @endif
                                        @else
                                            {{ now()->locale('id')->diffForHumans($contract->tgl_mulai_kontrak->locale('id'), ['parts' => 3, 'short' => false, 'syntax' => \Carbon\CarbonInterface::DIFF_ABSOLUTE]) }}
                                        @endif
                                    </p>
